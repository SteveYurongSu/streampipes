...
import org.apache.streampipes.dataexplorer.iotdb.v0.AutoAggregationHandler;
...
public class QueryResultProvider {
  ...
  public SpQueryResult getData() {
    ...
    Map<String, IotdbQueryParams> queryParts = DataLakeManagementUtils.getSelectQueryParams(queryParams);
    ...
    if (queryParams.getProvidedParams().containsKey(QP_MAXIMUM_AMOUNT_OF_EVENTS)) {
      ...
      return new DataExplorerQuery(queryParts, maximumAmountOfEvents).executeQuery(ignoreMissingData);
    }

    if (queryParams.getProvidedParams().containsKey(FOR_ID_KEY)) {
      String forWidgetId = queryParams.getProvidedParams().get(FOR_ID_KEY);
      return new DataExplorerQuery(queryParts, forWidgetId).executeQuery(ignoreMissingData);
    } else {
      return new DataExplorerQuery(queryParts).executeQuery(ignoreMissingData);
    }
  }
}
