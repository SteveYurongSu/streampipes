...
import org.apache.streampipes.dataexplorer.iotdb.v0.params.TimeBoundaryParams;
...
public class TimeBoundary extends QueryElement<TimeBoundaryParams> {
  ...
  protected String buildStatement(TimeBoundaryParams timeBoundaryParams) {
    if (timeBoundaryParams.getStartDate() == null) {
      return QueryTemplates.whereTimeRightBound(timeBoundaryParams.getEndDate());
    } else if (timeBoundaryParams.getEndDate() == null) {
      return QueryTemplates.whereTimeLeftBound(timeBoundaryParams.getStartDate());
    } else {
      return QueryTemplates.whereTimeWithin(timeBoundaryParams.getStartDate(), timeBoundaryParams.getEndDate());
    }
  }
}
